...
{
    internal sealed class EventTotalAttendanceHandler
    {
        private readonly TotalAttendanceService _service;

        internal EventTotalAttendanceHandler(TotalAttendanceService service)
        {
        ...
        internal async Task HandleCheckThisWeekAsync(
            EnhancedClientSession session,
            ...
            byte[] body)
        {
            if (!TryGetIdentity(
                    ...
                    out var characterId))
            {
                await SendAckAsync(session, success: false);
                FileLogger.Log(
                    "[TotalAttendance] rejected check without active character");
                return;
            }

            var result = _service.CheckThisWeek(
                accountId,
                characterId,
                DecodeCharacterName(session),
                session.Player?.Level ?? 0);

            await SendAckAsync(session, result.Success);
            if (result.MailDelivered)
                await SendMailboxAlarmAsync(session);
            if (result.Snapshot != null)
                await SendStateAsync(session, result.Snapshot, "check");

            if (!result.Success)
            {
                FileLogger.Log(
                    "[TotalAttendance] check skipped "
        ...
        internal static Task<bool> SendStateAsync(
            EnhancedClientSession session,
            TotalAttendanceSnapshot snapshot,
            string reason)
        {
            if (session == null
                || snapshot == null
            ...
            bool success)
        {
            ...
            return session.SendPacketAsync(
                TotalAttendancePacketBuilder.BuildCheckThisWeekAckPacket(
                    success ? 0u : 1u));
        }
