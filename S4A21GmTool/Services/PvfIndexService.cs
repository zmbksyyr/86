...
{
    // 从 Script.pvf 建物品/任务/套装索引，用于搜索、预览和发放前校验。
    // 索引建完就释放归档本体；常驻内存是物品、套装、任务字典。
    // partial 按域拆分: Items Jobs Quests World Sets Preview/PreviewStats
    public sealed partial class PvfIndexService
    {
        ...
        private void Build()
        {
            ...
            var setCount = _setsById.Count;
            Console.WriteLine($"[PvfIndex] 索引就绪: 物品 {itemNames.Count}, 套装 {setCount}, 任务 {(_questMeta != null ? _questMeta.Count : 0)}"
