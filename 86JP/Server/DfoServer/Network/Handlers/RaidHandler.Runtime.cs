using System;using System.Collections.Generic;using System.Threading.Tasks;using DfoServer.Game.Raid;using DfoServer.Network.Builders;using DfoServer.Network;using DfoServer.Network.Builders.Raid;namespace DfoServer.Network.Handlers;public sealed partial class RaidHandler{
	...
	await SetSymbolIfChangedAsync(current, 1u, 1u, "black-fog-passive");
			}
		}
	...
	await SetSymbolIfChangedAsync(current, 2u, 1u, "naval-meteo");
			}
		}
	...
	private async Task<bool> SetSymbolIfChangedAsync(RaidSnapshot raid, uint symbolId, uint value, string reason)	{
		var key = (raid.RaidId, symbolId);
		while (true)
		{
			if (_symbolValues.TryGetValue(key, out var currentValue))
			{
				if (currentValue == value)
				{
					FileLogger.Log($"[GameProtocol] RAID_SYMBOL_DUPLICATE_SUPPRESSED raid={raid.RaidId} symbol={symbolId} value={value} reason={reason}");
					return false;
				}
				if (!_symbolValues.TryUpdate(key, value, currentValue))
					continue;
			}
			else if (!_symbolValues.TryAdd(key, value))
			{
				continue;
			}
			break;
		}
		await BroadcastRaidNotificationAsync(raid, NotiPacketType.RAID_SET_SYMBOL, RaidPacketBuilder.BuildSetSymbol(symbolId, value));
		return true;
	}
	...
	RunInBackground(
	RunAttackTimeoutAsync(raid.RaidId, raid.PhaseIndex, remainingSeconds, version),			"raid-attack-timeout");
	}
	private async Task RunAttackTimeoutAsync(		uint raidId,		uint phaseIndex,		uint remainingSeconds,		int version)	{
		try		{

	await Task.Delay(checked((int)remainingSeconds * 1000));

	if (!TimerCurrent(raidId, AttackTimerType, AttackTimerDungeonId, version)				|| !_raids.TryFailPhase(raidId, phaseIndex, out var failed))			{
				return;
			}
	...
	CancelAllPhaseTwoTimers(raidId);
		...
		uint[] activeDungeonIds = phaseIndex == 0 ? AntonFirstPhaseDungeonIds : AntonSecondPhaseDungeonIds;
	foreach (uint dungeonId in activeDungeonIds)
		await SetDungeonStateAsync(failed, dungeonId, 2u);
		IReadOnlyList<KeyValuePair<NotiPacketType, byte[]>> terminalPackets = BuildAttackTimeoutTerminalPackets(failed);

	for (int packetIndex = 0; packetIndex < terminalPackets.Count; packetIndex++)			{

	KeyValuePair<NotiPacketType, byte[]> packet = terminalPackets[packetIndex];

	FileLogger.Log(
		$"[GameProtocol] RAID_ATTACK_TIMEOUT_SEND raid={raidId} " +
		$"noti={(ushort)packet.Key} body={BitConverter.ToString(packet.Value)}");
	await BroadcastRaidNotificationAsync(failed, packet.Key, packet.Value);
			}
	...
	FileLogger.Log(				$"[GameProtocol] RAID_ATTACK_TIMEOUT raid={raidId} phase={phaseIndex} " +				$"elapsed={failed.PhaseClearTimeSeconds} deaths={failed.PhaseDeathCount}");
		}
	...
	internal static IReadOnlyList<KeyValuePair<NotiPacketType, byte[]>> BuildAttackTimeoutTerminalPackets(		RaidSnapshot failed)	{

	if (failed == null)
	throw new ArgumentNullException(nameof(failed));

	var packets = new List<KeyValuePair<NotiPacketType, byte[]>>(4);

	// Establish the failed result context before 104 notifies the banner listener.
	if (failed.PhaseIndex == 0)
	{
		...
		packets.Add(new KeyValuePair<NotiPacketType, byte[]>(
			...
			RaidPacketBuilder.BuildRaidResult(1u, failed.PhaseIndex, failed.PhaseClearTimeSeconds, failed.PhaseDeathCount, 0u, 1)));
		packets.Add(new KeyValuePair<NotiPacketType, byte[]>(
			NotiPacketType.RAID_SET_SYMBOL,
			RaidPacketBuilder.BuildSetSymbol(AntonPhaseOneFailSymbolId, 1u)));
		packets.Add(new KeyValuePair<NotiPacketType, byte[]>(
			NotiPacketType.RAID_STATE,
			RaidPacketBuilder.BuildRaidState(3u, 0u)));
		return packets;
	}
	...
	packets.Add(new KeyValuePair<NotiPacketType, byte[]>(
		...
		RaidPacketBuilder.BuildRaidResult(
			resultType: 1u,
			phaseIndex: failed.PhaseIndex,
			clearTimeSeconds: failed.PhaseClearTimeSeconds,
			deadCount: failed.PhaseDeathCount,
			rank: 0u,
			rewardOption: 1)));
	packets.Add(new KeyValuePair<NotiPacketType, byte[]>(
		NotiPacketType.RAID_STATE,
	...
	_phaseOneResetClaims.TryRemove(raidId, out var _);
