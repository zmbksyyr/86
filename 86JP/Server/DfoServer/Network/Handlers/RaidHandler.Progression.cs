using System;using System.Collections.Generic;using System.Linq;using System.Threading.Tasks;using DfoServer.Game.Dungeon;using DfoServer.Game.Inventory;using DfoServer.Game.Raid;using DfoServer.Network.Builders;using DfoServer.Network.Builders.Raid;namespace DfoServer.Network.Handlers;public sealed partial class RaidHandler{
	...
	await BroadcastRaidNotificationAsync(started, NotiPacketType.RAID_SET_TIMER, RaidPacketBuilder.BuildSetTimer(0u, 0u, PhaseTwoTestAttackSeconds));

	await BroadcastRaidNotificationAsync(started, NotiPacketType.RAID_REMAIN_TIME, RaidPacketBuilder.BuildRemainTime(0, PhaseTwoTestAttackSeconds));

	StartAttackTimeoutTimer(started, PhaseTwoTestAttackSeconds);

	await StartHatcheryOpenTimerAsync(started);

	StartBarrierRecoveryTimer(started);

	FileLogger.Log($"[GameProtocol] START_RAID_PHASE2_ATTACK raid={started.RaidId} reason={reason} seconds={PhaseTwoTestAttackSeconds}");
	}
	...
	if (!IsAntonRaidDungeon(dungeonId)			|| !TryResolveUserId(session, out var userId)			|| !_raids.TryGetByUser(userId, out var currentRaid)			|| !IsAntonDungeonForPhase(currentRaid.PhaseIndex, dungeonId)			|| !_raids.TryRecordDeath(userId, out var raid))			return;

	RaidMember deadMember = raid.Members.FirstOrDefault(member => member.UserId == userId);

	if (deadMember == null)			return;

	await BroadcastRaidNotificationAsync(			raid,			NotiPacketType.RAID_MEMBER_STATE,
	RaidPacketBuilder.BuildRaidMemberState(deadMember.UserId, RaidMemberStateDead));

	FileLogger.Log(			$"[GameProtocol] RAID_PHASE_DEATH raid={raid.RaidId} phase={raid.PhaseIndex} " +			$"user={userId} dungeon={dungeonId} deaths={raid.PhaseDeathCount}");
	}
	public async Task HandleDungeonCharacterReviveAsync(		EnhancedClientSession session,		int dungeonId,		ushort targetActorId)	{

	if (!IsAntonRaidDungeon(dungeonId)			|| !TryResolveUserId(session, out var usingUserId)			|| !_raids.TryGetByUser(usingUserId, out var currentRaid)			|| !IsAntonDungeonForPhase(currentRaid.PhaseIndex, dungeonId)			|| !currentRaid.Members.Any(member => member.UserId == targetActorId)			|| !_raids.TryRecordCoinUse(usingUserId, checked((uint)dungeonId), out var raid))			return;
	...
	await BroadcastRaidNotificationAsync(			raid,			NotiPacketType.RAID_MEMBER_STATE,
	RaidPacketBuilder.BuildRaidMemberState(revivedMember.UserId, RaidMemberStateAlive));

	await BroadcastRaidMonsterStatusAsync(raid);

	FileLogger.Log(			$"[GameProtocol] RAID_PHASE_REVIVE raid={raid.RaidId} phase={raid.PhaseIndex} " +			$"user={targetUserId} actor={targetActorId} coinUser={usingUserId} dungeon={dungeonId}");
	}
	...
	_phaseOneResetClaims.TryRemove(raid.RaidId, out var _);
	...
	if (!_phaseOneResetClaims.TryAdd(raid.RaidId, 0))
	{
		FileLogger.Log($"[GameProtocol] RAID_PHASE1_FAIL_RESET duplicate-suppressed raid={raid.RaidId}");
		return;
	}
	...
	_raids.ResetClearCounts(raid.RaidId, AntonFirstPhaseDungeonIds);
		// 104 is the confirmed phase-one failure/reset banner symbol.
	await SetSymbolAsync(raid, AntonPhaseOneFailSymbolId, 1u);
	...
	where _raids.HasClearedDungeon(result.RaidId, member.UserId)				select member.UserId).ToArray();
	...
	if (_raids.TryEnterPhaseOneBreak(raid.RaidId, 213u, 2u, 215u, 2u, out var waiting))		{
	...
	where _raids.HasClearedDungeon(waiting.RaidId, member.UserId)				select member.UserId).ToArray();
